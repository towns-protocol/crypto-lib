...
library SCL_ECCUTILS {
    ...
    function ecAddn(
        ...
        assembly {
            function ecAddn2(x1, y1, zz1, zzz1, x2, y2, _p)
                -> _x, _y, _zz, _zzz
            {
                y1 := sub(_p, y1)
                ...
                _x := addmod(
                    addmod(mulmod(y2, y2, _p), sub(_p, _y), _p),
                    mulmod(sub(_p, 2), zz1, _p),
                    _p
                ) //R^2-PPP-2*Q
    ...
    function EcDbl(
        ...
        uint256 X,
        uint256 Y
    ) public view returns (uint256 dX, uint256 dY) {
        assembly {
            function ecDbl(x, y, zz, zzz, _p, _a) -> _x, _y, _zz, _zzz {
                ...
                _y := addmod(
                    mulmod(3, mulmod(x, x, _p), _p),
                    mulmod(_a, mulmod(zz, zz, _p), _p),
                    _p
                ) //M=3*X12+aZZ12
    ...
    function SetKey(
        ...
        uint256 qx,
        uint256 qy
    ) public view returns (bool status, uint256[10] memory ExtendedKey) {
