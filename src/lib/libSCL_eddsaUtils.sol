...
library SCL_EIP6565_UTILS {
    ...
    function SetKey(
        ...
        public
        view
        returns (uint256[5] memory extKpub, uint256[2] memory signer)
    {
        ...
        (extKpub[0], extKpub[1]) = SCL_EIP6565.Edwards2WeierStrass(
            Kpub[0],
            Kpub[1]
        );
        (extKpub[2], extKpub[3]) = SCL_EIP6565.ecPow128(
            extKpub[0],
            extKpub[1],
            1,
            1
    ...
    function ExpandSecret(
        uint256 secret
    ) public view returns (uint256 KpubC, uint256 expanded) {
        uint256[2] memory Kpub;
